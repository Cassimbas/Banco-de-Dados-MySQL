...
public class Clientes2 extends JDialog {
	...
	public Clientes2() {
		...
		lblNewLabel_8.setBounds(513, 257, 118, 14);
		...
		lblNewLabel_12.setBounds(331, 213, 46, 14);
		getContentPane().add(lblNewLabel_12);
		
		txtCpf = new JTextField();
		txtCpf.setBounds(405, 210, 118, 20);
	...
	private void pesquisarCliente() {
		// ? é um parâmetro
		String read = "select idcli as ID, nome as Cliente,  fone1 as Fone, cpf as CPF, endereco as Endereço, numero as Número, complemento as Complemento, bairro as Bairro, cidade as Cidade, Uf as UF, Cep as Cep from clientes where nome like ?";
		try {
	...
	private void adicionarCliente() {
		...
		if (txtNome.getText().isEmpty()) {
			...
			String create = "insert into clientes(nome,cpf,fone1,endereco,numero,complemento,bairro,cidade,Uf,cep) values (?,?,?,?,?,?,?,?,?,?)";
			try {
				Connection con = dao.conectar();
	...
	private void setarCampos() {
		...
		txtCep.setText(table.getModel().getValueAt(setar, 10).toString());
		// Gerenciar os botões
	...
	private void editarCliente() {
		if (txtNome.getText().isEmpty()) {
			...
			String update = "update clientes set nome=?,fone1=?,cpf=?,endereco=?,numero=?,complemento=?,bairro=?,cidade=?,Uf=?,Cep=? where idcli=?";
			try {
				Connection con = dao.conectar();
